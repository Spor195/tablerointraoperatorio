...
# Finalidad: Registrar hitos de tiempo, calcular TAT y monitorear SLA.
...
def compute_metrics(df: pd.DataFrame, sla_min: int) -> pd.DataFrame:
    """
    Calcula TAT y derivados de forma vectorizada.
    Tolera DataFrames vacíos y tiempos faltantes (NaT).
    """
    df = df.copy()

    if df.empty:
        # Crear columnas vacías con dtypes adecuados
    ...
    # Parseo robusto a datetime (ISO con tz o vacío → NaT)
    ...
    # Cumplimiento y estado derivado
...
with tab_dia:
    ...
    if df2.empty:
        ...
        # Parseo y filtro solo si existen fechas válidas de recepción
        ...
        if df2["t_recepcion_dt"].notna().any():
            ...
            df2 = df2.iloc[0:0]  # sin fechas válidas → vacío
